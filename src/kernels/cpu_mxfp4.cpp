        ...
        for (uint32_t byte_index = 0; byte_index < 16; ++byte_index) {
            ...
            for (size_t token_index = 0; token_index < token_count; ++token_index) {
                ...
                output[token_index * output_stride]
                    += low * token[column] + high * token[column + 1];
            }
        }
    }
}

static void scalar_matmul_rows2(
        ...
        for (uint32_t byte_index = 0; byte_index < 16; ++byte_index) {
            ...
            for (size_t token_index = 0; token_index < token_count; ++token_index) {
                ...
                first_output[token_index * first_output_stride]
                    += first_low * low_input + first_high * high_input;
                second_output[token_index * second_output_stride]
                    += second_low * low_input + second_high * high_input;
            }
        }
    }
}
...
#if defined(__aarch64__) || defined(_M_ARM64)
static float neon_dot(
    const uint8_t* packed,
    const uint8_t* scales,
    uint32_t block_count,
    const float* input) noexcept
{
    static constexpr int8_t value_bytes[16] = {
        0, 1, 2, 3, 4, 6, 8, 12,
        0, -1, -2, -3, -4, -6, -8, -12};
    ...
    for (uint32_t block_index = 0; block_index < block_count; ++block_index) {
        ...
        for (size_t token_index = 0; token_index < token_count; ++token_index) {
            ...
            for (uint32_t group = 0; group < 4; ++group) {
                accumulator = vfmaq_f32(
                    accumulator,
                    vcvtq_f32_s32(vmovl_s16(vget_low_s16(decoded_16[group]))),
                    vld1q_f32(token + group * 8));
                accumulator = vfmaq_f32(
                    accumulator,
                    vcvtq_f32_s32(vmovl_s16(vget_high_s16(decoded_16[group]))),
                    vld1q_f32(token + group * 8 + 4));
            }
            output[token_index * output_stride]
                += vaddvq_f32(accumulator) * scale;
        }
    }
}

static void neon_matmul_rows2(
    const uint8_t* first_packed,
    const uint8_t* first_scales,
    const uint8_t* second_packed,
    const uint8_t* second_scales,
    ...
    for (uint32_t block_index = 0; block_index < block_count; ++block_index) {
        ...
        for (uint32_t row = 0; row < 2; ++row) {
            ...
            const uint8x16_t high = vandq_u8(
                vshrq_n_u8(packed_rows[row], 4), nibble_mask);
            const uint8x16x2_t interleaved = vzipq_u8(low, high);
            const int8x16_t decoded_low
                = vqtbl1q_s8(value_table, interleaved.val[0]);
            const int8x16_t decoded_high
                = vqtbl1q_s8(value_table, interleaved.val[1]);
        ...
        for (size_t token_index = 0; token_index < token_count; ++token_index) {
            ...
            for (uint32_t group = 0; group < 4; ++group) {
                ...
                const float32x4_t input_high
                    = vld1q_f32(token + group * 8 + 4);
                first_accumulator = vfmaq_f32(
                    first_accumulator,
                    vcvtq_f32_s32(
                        vmovl_s16(vget_low_s16(decoded_rows[0][group]))),
                    input_low);
                first_accumulator = vfmaq_f32(
                    first_accumulator,
                    vcvtq_f32_s32(
                        vmovl_s16(vget_high_s16(decoded_rows[0][group]))),
                    input_high);
                second_accumulator = vfmaq_f32(
                    second_accumulator,
                    vcvtq_f32_s32(
                        vmovl_s16(vget_low_s16(decoded_rows[1][group]))),
                    input_low);
                second_accumulator = vfmaq_f32(
                    second_accumulator,
                    vcvtq_f32_s32(
                        vmovl_s16(vget_high_s16(decoded_rows[1][group]))),
                    input_high);
            }
            first_output[token_index * first_output_stride]
                += vaddvq_f32(first_accumulator) * first_scale;
            second_output[token_index * second_output_stride]
                += vaddvq_f32(second_accumulator) * second_scale;
        }
    }
}
#endif
    ...
    const float* input) noexcept
{
    ...
    for (uint32_t block_index = 0; block_index < block_count; ++block_index) {
        const __m128i bytes = _mm_loadu_si128(
            reinterpret_cast<const __m128i*>(packed + static_cast<size_t>(block_index) * 16));
    ...
    for (uint32_t block_index = 0; block_index < block_count; ++block_index) {
        ...
        for (size_t token_index = 0; token_index < token_count; ++token_index) {
            ...
            for (uint32_t half = 0; half < 2; ++half) {
                ...
                accumulator = _mm256_fmadd_ps(
                    _mm256_cvtepi32_ps(
                        _mm256_cvtepi8_epi32(_mm_srli_si128(decoded[half], 8))),
            ...
            const __m128 pairs = _mm_hadd_ps(halves, halves);
            output[token_index * output_stride]
                += _mm_cvtss_f32(_mm_hadd_ps(pairs, pairs)) * scale;
        }
    }
}
    ...
    size_t second_output_stride) noexcept
{
    ...
    for (uint32_t block_index = 0; block_index < block_count; ++block_index) {
        ...
        for (uint32_t row = 0; row < 2; ++row) {
            ...
            const __m128i high = _mm_and_si128(
                _mm_srli_epi16(packed_rows[row], 4), nibble_mask);
            decoded_rows[row][0] = _mm_shuffle_epi8(
                value_table, _mm_unpacklo_epi8(low, high));
            decoded_rows[row][1] = _mm_shuffle_epi8(
        ...
        for (size_t token_index = 0; token_index < token_count; ++token_index) {
            ...
            const __m128 first_halves = _mm_add_ps(
                _mm256_castps256_ps128(first_accumulator),
                _mm256_extractf128_ps(first_accumulator, 1));
            const __m128 first_pairs
                = _mm_hadd_ps(first_halves, first_halves);
            const float first_sum = _mm_cvtss_f32(
                _mm_hadd_ps(first_pairs, first_pairs));
            const __m128 second_halves = _mm_add_ps(
                _mm256_castps256_ps128(second_accumulator),
                _mm256_extractf128_ps(second_accumulator, 1));
            const __m128 second_pairs
                = _mm_hadd_ps(second_halves, second_halves);
            const float second_sum = _mm_cvtss_f32(
                _mm_hadd_ps(second_pairs, second_pairs));
            first_output[token_index * first_output_stride]
                += first_sum * first_scale;
            second_output[token_index * second_output_stride]
                += second_sum * second_scale;
        }
    }
}
    ...
    const float* input) noexcept
{
    ...
    for (uint32_t block_index = 0; block_index < block_count; ++block_index) {
        ...
        const __m128i low = _mm_and_si128(bytes, nibble_mask);
        const __m128i high = _mm_and_si128(_mm_srli_epi16(bytes, 4), nibble_mask);
        const __m128i decoded_low = _mm_shuffle_epi8(
            value_table, _mm_unpacklo_epi8(low, high));
        const __m128i decoded_high = _mm_shuffle_epi8(
            value_table, _mm_unpackhi_epi8(low, high));
        const float* input_block = input + static_cast<size_t>(block_index) * 32;
        __m512 accumulator = _mm512_mul_ps(
            _mm512_cvtepi32_ps(_mm512_cvtepi8_epi32(decoded_low)),
            _mm512_loadu_ps(input_block));
        accumulator = _mm512_fmadd_ps(
            _mm512_cvtepi32_ps(_mm512_cvtepi8_epi32(decoded_high)),
            _mm512_loadu_ps(input_block + 16),
            accumulator);
        sum += _mm512_reduce_add_ps(accumulator)
               * (0.5f * scales_by_exponent[scales[block_index]]);
    }
    return sum;
}
    ...
    size_t output_stride) noexcept
{
    ...
    for (uint32_t block_index = 0; block_index < block_count; ++block_index) {
        ...
        const __m128i decoded_high = _mm_shuffle_epi8(
            value_table, _mm_unpackhi_epi8(low, high));
        const __m512 weights = _mm512_cvtepi32_ps(
            _mm512_cvtepi8_epi32(decoded_low));
        const __m512 weights_high = _mm512_cvtepi32_ps(
            _mm512_cvtepi8_epi32(decoded_high));
        const size_t input_offset = static_cast<size_t>(block_index) * 32;
        const float scale
            = 0.5f * scales_by_exponent[scales[block_index]];
        for (size_t token_index = 0; token_index < token_count; ++token_index) {
            ...
            const __m512 accumulator = _mm512_fmadd_ps(
                ...
                _mm512_mul_ps(weights, _mm512_loadu_ps(token)));
            output[token_index * output_stride]
    ...
    size_t second_output_stride) noexcept
{
    ...
    for (uint32_t block_index = 0; block_index < block_count; ++block_index) {
        ...
        for (uint32_t row = 0; row < 2; ++row) {
            ...
            const __m128i high = _mm_and_si128(
                _mm_srli_epi16(packed_rows[row], 4), nibble_mask);
            decoded_low[row] = _mm512_cvtepi32_ps(_mm512_cvtepi8_epi32(
                _mm_shuffle_epi8(
                    value_table, _mm_unpacklo_epi8(low, high))));
            decoded_high[row] = _mm512_cvtepi32_ps(_mm512_cvtepi8_epi32(
                _mm_shuffle_epi8(
                    value_table, _mm_unpackhi_epi8(low, high))));
        }
        const size_t input_offset = static_cast<size_t>(block_index) * 32;
        ...
        for (size_t token_index = 0; token_index < token_count; ++token_index) {
            ...
            const __m512 first_accumulator = _mm512_fmadd_ps(
                decoded_high[0],
                input_high,
                _mm512_mul_ps(decoded_low[0], input_low));
            const __m512 second_accumulator = _mm512_fmadd_ps(
                decoded_high[1],
                input_high,
                _mm512_mul_ps(decoded_low[1], input_low));
            first_output[token_index * first_output_stride]
...
using DotFunction = float (*)(
    const uint8_t*,
    const uint8_t*,
    uint32_t,
    const float*) noexcept;

using GemmRowFunction = void (*)(
    const uint8_t*,
    const uint8_t*,
    uint32_t,
    const float*,
...
using MatmulRows2Function = void (*)(
    const uint8_t*,
    const uint8_t*,
    const uint8_t*,
    const uint8_t*,
...
const char* mxfp4_kernel_name() noexcept
{
    ...
    case MxFp4KernelKind::ArmNeon:
        return "arm-neon";
    case MxFp4KernelKind::X86Avx2:
        return "x86-avx2-fma";
    case MxFp4KernelKind::X86Avx512:
        return "x86-avx512";
    case MxFp4KernelKind::Scalar:
        return "scalar";
    }
    return "scalar";
}
