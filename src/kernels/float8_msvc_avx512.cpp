...
static __forceinline __m512bh decode_float8_e4m3_bfloat16(__m256i packed) noexcept
{
    const __m512i bytes = _mm512_cvtepu8_epi16(packed);
    const __m512i exponent = _mm512_and_si512(_mm512_srli_epi16(bytes, 3), _mm512_set1_epi16(15));
    const __m512i mantissa = _mm512_and_si512(bytes, _mm512_set1_epi16(7));
    const __m512i sign = _mm512_slli_epi16(_mm512_and_si512(bytes, _mm512_set1_epi16(128)), 8);
    const __m512i normal = _mm512_or_si512(sign,
                                           _mm512_or_si512(_mm512_slli_epi16(_mm512_add_epi16(exponent, _mm512_set1_epi16(120)), 7),
                                                           _mm512_slli_epi16(mantissa, 4)));
    const __m512i subnormal_table = _mm512_setr_epi16(0x0000, 0x3b00, 0x3b80, 0x3bc0,
                                                      ...
                                                      0x0000, 0x3b00, 0x3b80, 0x3bc0,
                                                      0x3c00, 0x3c20, 0x3c40, 0x3c60);
    const __m512i subnormal = _mm512_or_si512(sign, _mm512_permutexvar_epi16(mantissa, subnormal_table));
    const __mmask32 exponent_zero = _mm512_cmpeq_epi16_mask(exponent, _mm512_setzero_si512());
    __m512i decoded = _mm512_mask_blend_epi16(exponent_zero, normal, subnormal);
    const __mmask32 nan_mask = _mm512_cmpeq_epi16_mask(exponent, _mm512_set1_epi16(15))
                               & _mm512_cmpeq_epi16_mask(mantissa, _mm512_set1_epi16(7));
    decoded = _mm512_mask_blend_epi16(nan_mask, decoded,
                                      _mm512_or_si512(sign, _mm512_set1_epi16(0x7fc0)));
    return (__m512bh)decoded;
}

static __forceinline __m512bh pack_input_bfloat16(const float* input) noexcept
{
    return _mm512_cvtne2ps_pbh(_mm512_loadu_ps(input + 16),
                               _mm512_loadu_ps(input));
}

void msvc_avx512_quantize_float8_e4m3(const float* source,
                                      float* values,
                                      uint32_t count,
                                      uint32_t block_size,
                                      bool power_of_two_scale) noexcept
{
    const __m512 absolute_mask = _mm512_castsi512_ps(_mm512_set1_epi32(0x7fffffff));
    ...
    const __m512 canonical_nan = _mm512_castsi512_ps(_mm512_set1_epi32(0x7fc00000));
         ...
         block_begin += block_size)
    {
        ...
        {
            const __m512 magnitude = _mm512_and_ps(_mm512_loadu_ps(source + index), absolute_mask);
            const __mmask16 greater = _mm512_cmp_ps_mask(magnitude, vector_maximum, _CMP_GT_OQ);
            vector_maximum = _mm512_mask_mov_ps(vector_maximum, greater, magnitude);
        }
        ...
        for (; index + 16 <= block_end; index += 16)
        {
            ...
            const __mmask16 nan_mask = _mm512_cmp_ps_mask(magnitude, magnitude, _CMP_UNORD_Q);
            const __mmask16 above_maximum = _mm512_cmp_ps_mask(magnitude, maximum_value, _CMP_GT_OQ);
            magnitude = _mm512_mask_mov_ps(magnitude, above_maximum, maximum_value);

            const __m512i rounded_bits = _mm512_and_si512(_mm512_add_epi32(_mm512_castps_si512(magnitude), rounding_bias),
                                                          e4m3_value_mask);
            const __m512 normal = _mm512_castsi512_ps(rounded_bits);
            __m512 subnormal = _mm512_floor_ps(_mm512_add_ps(_mm512_mul_ps(magnitude, subnormal_scale), half));
            const __mmask16 above_subnormal_maximum = _mm512_cmp_ps_mask(subnormal, subnormal_maximum, _CMP_GT_OQ);
            subnormal = _mm512_mask_mov_ps(subnormal, above_subnormal_maximum, subnormal_maximum);
            subnormal = _mm512_mul_ps(subnormal, subnormal_inverse_scale);
            const __mmask16 normal_mask = _mm512_cmp_ps_mask(magnitude, minimum_normal, _CMP_GE_OQ);
            __m512 quantized = _mm512_mask_blend_ps(normal_mask, subnormal, normal);
            quantized = _mm512_castsi512_ps(_mm512_xor_si512(_mm512_castps_si512(quantized), signs));
            quantized = _mm512_mul_ps(quantized, scale_vector);
            quantized = _mm512_mask_mov_ps(quantized, nan_mask, canonical_nan);
        ...
        for (; index < block_end; ++index)
        {
            ...
            values[index] = float8_e4m3_to_float(float_to_float8_e4m3(normalized))
                            * scale;
        }
    }
}
...
float msvc_avx512_bfloat16_float8_e4m3_block_dot(const uint8_t* weights,
                                                 const float* scales,
                                                 const float* input,
                                                 uint32_t count,
...
static void bfloat16_float8_e4m3_block_dot_rows8(const uint8_t* weights,
...
#define NCNN_MOE_ACCUMULATE_FLOAT8_ROW(row)                                                                                                                                    \
    accumulator##row = _mm512_dpbf16_ps(accumulator##row,                                                                                                                      \
                                        decode_float8_e4m3_bfloat16(_mm256_loadu_si256(reinterpret_cast<const __m256i*>(weights + static_cast<size_t>(row) * weight_row_stride \
                                                                                                                        + index))),                                            \
                                        input_values)
...
#undef NCNN_MOE_ACCUMULATE_FLOAT8_ROW
        ...
        for (uint32_t row = 0; row < 8; ++row)
        {
            for (uint32_t tail = index; tail < block_end; ++tail)
            {
                partials[row] += float8_e4m3_to_float(weights[static_cast<size_t>(row)
                                                                  * weight_row_stride
                                                              + tail])
                                 * input[tail];
            }
...
void msvc_avx512_bfloat16_float8_e4m3_block_dot_rows(const uint8_t* weights,
                                                     uint32_t weight_row_stride,
                                                     const float* scales,
                                                     const float* input,
                                                     uint32_t count,
                                                     ...
                                                     float* output) noexcept
{
    ...
    {
        bfloat16_float8_e4m3_block_dot_rows8(weights, weight_row_stride, scales, input, count, block_size,
                                             output);
        return;
    }
    for (uint32_t row = 0; row < row_count; ++row)
         ...
         block_begin += block_size, ++block)
    {
        ...
        for (; index + 32 <= block_end; index += 32)
        {
            ...
            for (uint32_t row = 0; row < row_count; ++row)
            {
                const __m256i packed = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(weights + static_cast<size_t>(row) * weight_row_stride
                                                                                           + index));
                accumulators[row] = _mm512_dpbf16_ps(accumulators[row],
        ...
        for (uint32_t row = 0; row < row_count; ++row)
        {
            ...
            for (uint32_t tail = index; tail < block_end; ++tail)
            {
                partial += float8_e4m3_to_float(weights[static_cast<size_t>(row)
                                                            * weight_row_stride
                                                        + tail])
                           * input[tail];
            }
...
void msvc_avx512_bfloat16_float8_e4m3_block_dot_rows_batch(const uint8_t* weights,
                                                           uint32_t weight_row_stride,
                                                           const float* scales,
                                                           const float* input,
                                                           size_t input_stride,
                                                           ...
                                                           float* output) noexcept
{
         ...
         block_begin += block_size, ++block)
    {
        ...
        for (; index + 32 <= block_end; index += 32)
        {
            ...
            for (size_t token = 0; token < token_count; ++token)
            {
                input_values[token] = pack_input_bfloat16(input + token * input_stride + index);
            }
            __m512bh weight_values[4];
            for (uint32_t row = 0; row < row_count; ++row)
            {
                const uint8_t* row_weights = weights + static_cast<size_t>(row) * weight_row_stride
                                             + index;
                weight_values[row] = decode_float8_e4m3_bfloat16(_mm256_loadu_si256(reinterpret_cast<const __m256i*>(row_weights)));
            }
            for (uint32_t row = 0; row < row_count; ++row)
            {
                for (size_t token = 0; token < token_count; ++token)
                {
                    accumulators[row][token] = _mm512_dpbf16_ps(accumulators[row][token], weight_values[row],
                                                                input_values[token]);
                }
            }
        }
        ...
        for (uint32_t row = 0; row < row_count; ++row)
        {
            ...
            for (size_t token = 0; token < token_count; ++token)
            {
                float partial = _mm512_reduce_add_ps(accumulators[row][token]);
                ...
                output[token * output_stride + row] += partial * scales[block];
            }
        }
    }
}
