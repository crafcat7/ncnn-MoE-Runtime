...
static __m256 msvc_avx2_expf(__m256 value) noexcept
{
    ...
    const __m256 remainder = _mm256_fnmadd_ps(exponent, _mm256_set1_ps(0x1.7f7d1cp-20f),
                              ...
                              size_t output_stride) noexcept
{
    ...
    for (; column + 8 <= input_columns; column += 8)
    {
        ...
        {
            input_values[token] = _mm256_loadu_ps(input + static_cast<size_t>(token) * input_stride + column);
        }
        for (uint32_t output_index = 0; output_index < output_count; ++output_index)
        {
            const __m256 weight_values = _mm256_loadu_ps(weights + static_cast<size_t>(output_index) * weight_stride + column);
            for (uint32_t token = 0; token < token_count; ++token)
            {
                accumulators[token][output_index] = _mm256_fmadd_ps(input_values[token],
                                                                    weight_values,
                              ...
                              size_t output_stride) noexcept
{
    ...
    for (; column + 8 <= input_columns; column += 8)
    {
        ...
        for (uint32_t output_index = 0; output_index < output_count; ++output_index)
        {
            const __m256 weight_values = _mm256_loadu_ps(weights + static_cast<size_t>(output_index) * weight_stride + column);
            for (uint32_t token = 0; token < token_count; ++token)
                accumulators[token][output_index] = _mm256_fmadd_ps(input_values[token], weight_values,
                                                                    accumulators[token][output_index]);
        }
    }
                                 ...
                                 size_t output_stride) noexcept
{
    ...
    for (; column + 8 <= input_columns; column += 8)
    {
        ...
        for (uint32_t output_index = 0; output_index < output_count; ++output_index)
        {
            const __m128i packed = _mm_loadu_si128(reinterpret_cast<const __m128i*>(weights + static_cast<size_t>(output_index) * weight_stride + column));
            weight_values[output_index] = _mm256_castsi256_ps(_mm256_slli_epi32(_mm256_cvtepu16_epi32(packed), 16));
            for (uint32_t token = 0; token < token_count; ++token)
                accumulators[token][output_index] = _mm256_fmadd_ps(input_values[token], weight_values[output_index],
                                                                    accumulators[token][output_index]);
        }
    }
                               ...
                               uint32_t count) noexcept
{
    ...
    for (; index + 32 <= count; index += 32)
    {
        accumulator0 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(_mm_loadl_epi64(reinterpret_cast<const __m128i*>(left + index)))),
                                       _mm256_loadu_ps(right + index),
                                       accumulator0);
        accumulator1 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(_mm_loadl_epi64(reinterpret_cast<const __m128i*>(left + index + 8)))),
                                       _mm256_loadu_ps(right + index + 8),
                                       accumulator1);
        accumulator2 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(_mm_loadl_epi64(reinterpret_cast<const __m128i*>(left + index + 16)))),
                                       _mm256_loadu_ps(right + index + 16),
                                       accumulator2);
        accumulator3 = _mm256_fmadd_ps(_mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(_mm_loadl_epi64(reinterpret_cast<const __m128i*>(left + index + 24)))),
                                       _mm256_loadu_ps(right + index + 24),
                                       accumulator3);
    }
    __m256 sum = _mm256_add_ps(_mm256_add_ps(accumulator0, accumulator1),
                               _mm256_add_ps(accumulator2, accumulator3));
    for (; index + 8 <= count; index += 8)
    {
        sum = _mm256_fmadd_ps(_mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(_mm_loadl_epi64(reinterpret_cast<const __m128i*>(left + index)))),
                              _mm256_loadu_ps(right + index),
                              sum);
    }
                               ...
                               uint32_t count) noexcept
{
    ...
    for (; index + 8 <= count; index += 8)
    {
        const __m256 scaled_output = _mm256_mul_ps(_mm256_loadu_ps(output + index), output_scale_values);
        const __m256 scaled_input = _mm256_mul_ps(_mm256_loadu_ps(input + index), input_scale_values);
                                                  ...
                                                  uint32_t count) noexcept
{
    ...
    for (; index + 8 <= count; index += 8)
    {
        const __m256 scaled_values = _mm256_mul_ps(_mm256_loadu_ps(values + index), value_scale_values);
                                                                 ...
                                                                 uint32_t count) noexcept
{
    ...
    {
        const __m256 scaled_values = _mm256_mul_ps(_mm256_loadu_ps(values + index), value_scale_values);
        const __m256 input_values = _mm256_mul_ps(_mm256_loadu_ps(input + index), input_scale_values);
        const __m256 updated_values = _mm256_add_ps(scaled_values, input_values);
        _mm256_storeu_ps(values + index, updated_values);
        _mm256_storeu_ps(output + index,
                         _mm256_add_ps(_mm256_loadu_ps(output + index),
                                       _mm256_mul_ps(updated_values, output_scale_values)));
    }
    for (; index < count; ++index)
    {
