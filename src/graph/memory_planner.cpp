...
static Result<uint64_t> latent_fp8_dense_size(const MoeIR& ir)
{
    uint64_t total = 0;
    Result<void> status = add_matrix_size(ir.vocabulary_size, ir.hidden_size, DType::BFloat16, "DeepSeek embedding", total);
    if (!status)
        return status.error();
    status = add_matrix_size(ir.vocabulary_size, ir.hidden_size, DType::BFloat16, "DeepSeek LM head", total);
    if (!status)
        return status.error();
    status = add_vector_size(ir.hidden_size, 2, "DeepSeek final norm", total);
    if (!status)
        return status.error();

    const uint64_t multiplier = ir.hyper_connection_multiplier;
    status = add_matrix_size(multiplier, multiplier * ir.hidden_size, DType::Float32, "DeepSeek hyper head", total);
    if (!status)
        return status.error();
    ...
    for (uint32_t layer_id = 0; layer_id < ir.layers.size(); ++layer_id)
    {
        ...
        {
            status = add_matrix_size(mix_count, multiplier * ir.hidden_size, DType::Float32, "DeepSeek hyper function", total);
            if (!status)
                return status.error();
        ...
        status = add_vector_size(ir.hidden_size * 2ull, 2, "DeepSeek layer norms", total);
        if (!status)
            return status.error();

        status = add_matrix_size(attention.query_lora_rank, ir.hidden_size, attention.projection_weight_dtype, "DeepSeek query A", total);
        if (!status)
            return status.error();
        ...
        if (!status)
            return status.error();
        status = add_matrix_size(attention.head_dimension, ir.hidden_size, attention.projection_weight_dtype, "DeepSeek latent KV", total);
        if (!status)
            return status.error();
        ...
        if (!status)
            return status.error();
        status = add_matrix_size(ir.hidden_size, static_cast<uint64_t>(attention.output_group_count) * attention.output_lora_rank, attention.projection_weight_dtype, "DeepSeek output B", total);
        if (!status)
            return status.error();
        ...
        if (attention.compression_ratio != 0)
        {
            ...
            status = add_matrix_size(compressor_multiplier * attention.head_dimension, ir.hidden_size, DType::BFloat16, "DeepSeek compressor KV", total);
            if (!status)
                return status.error();
            status = add_matrix_size(compressor_multiplier * attention.head_dimension, ir.hidden_size, DType::BFloat16, "DeepSeek compressor gate", total);
            if (!status)
                return status.error();
            ...
            {
                status = add_matrix_size(2ull * attention.index_head_dimension, ir.hidden_size, DType::BFloat16, "DeepSeek index compressor KV", total);
                if (!status)
                    return status.error();
                status = add_matrix_size(2ull * attention.index_head_dimension, ir.hidden_size, DType::BFloat16, "DeepSeek index compressor gate", total);
                if (!status)
                    return status.error();
                ...
                if (!status)
                    return status.error();
                status = add_matrix_size(attention.index_head_count, ir.hidden_size, DType::BFloat16, "DeepSeek index weights", total);
                if (!status)
                    return status.error();
        ...
        status = add_matrix_size(moe.expert_count, ir.hidden_size, DType::BFloat16, "DeepSeek router", total);
        if (!status)
            return status.error();
        if (layer_id < ir.hash_routing_layer_count)
            status = add_matrix_size(ir.vocabulary_size, moe.top_k, DType::Int64, "DeepSeek hash router", total);
        else
            status = add_vector_size(moe.expert_count, 4, "DeepSeek router bias", total);
        if (!status)
            return status.error();
        status = add_matrix_size(moe.intermediate_size, ir.hidden_size, moe.shared_expert_weight_dtype, "DeepSeek shared Expert input", total);
        if (!status)
            return status.error();
        status = add_matrix_size(moe.intermediate_size, ir.hidden_size, moe.shared_expert_weight_dtype, "DeepSeek shared Expert input", total);
        if (!status)
            return status.error();
        status = add_matrix_size(ir.hidden_size, moe.intermediate_size, moe.shared_expert_weight_dtype, "DeepSeek shared Expert output", total);
        if (!status)
            return status.error();
    }
    return total;
...
static Result<uint64_t> latent_vulkan_releasable_dense_size(const MoeIR& ir)
{
    if (ir.layers.empty()
...
static Result<uint64_t> dense_size(const MoeIR& ir)
{
    if (!ir.layers.empty()
        && ir.hyper_connection_multiplier > 1
        && ir.layers.front().attention.kind == AttentionKind::MultiHeadLatent
        && ir.layers.front().attention.projection_weight_dtype == DType::Float8E4M3)
        return latent_fp8_dense_size(ir);
    const uint64_t element_size = ir.activation_dtype == DType::BFloat16 ? 2 : 4;
    ...
    for (const LayerDescriptor& layer : ir.layers)
    {
        ...
        if (layer.ple.enabled())
        {
            ...
            const uint64_t expanded_size = static_cast<uint64_t>(ir.hyper_connection_multiplier)
                                           * ir.hidden_size;
...
static Result<uint64_t> mxfp4_pair_size(const MoeIR& ir)
{
...
static Result<uint64_t> expert_pair_size(const MoeIR& ir, DType dtype)
{
    if (dtype == DType::MxFp4)
        return mxfp4_pair_size(ir);
...
static Result<uint64_t> packed_expert_pair_size(const MoeIR& ir, DType dtype)
{
...
Result<ModelMemoryPlan> plan_model_memory(const MoeIR& ir, const Option& opt, uint64_t physical_memory_size,
                                          ...
                                          bool reserve_cpu_packed_weights)
{
    ...
    auto estimated_dense = dense_size(ir);
    if (!estimated_dense)
    ...
    auto pair_size = expert_pair_size(ir, moe.expert_weight_dtype);
    if (!pair_size)
    ...
    {
        auto packed_pair_size = packed_expert_pair_size(ir, moe.expert_weight_dtype);
        if (!packed_pair_size)
    ...
    {
        auto releasable = latent_vulkan_releasable_dense_size(ir);
        if (!releasable)
