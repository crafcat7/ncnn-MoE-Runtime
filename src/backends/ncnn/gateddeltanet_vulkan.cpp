...
static bool create_pipeline(const std::shared_ptr<VulkanContext>& context,
                            const ncnn::Option& opt,
                            std::shared_ptr<ncnn::Pipeline>& destination)
{
    const std::shared_ptr<const std::vector<uint32_t>> spirv = context->shader_binary(gated_delta_net_shader,
...
public:
    ...
    bool record_initial_state(ncnn::VkCompute& cmd)
    {
        ...
        if (initial_convolution.empty() || initial_recurrent.empty())
            return false;
        cmd.record_clone(convolution,
                         initial_convolution,
                         opt);
        cmd.record_clone(recurrent,
                         initial_recurrent,
                         opt);
    ...
    bool restore_snapshot(const ncnn::VkMat& snapshot_convolution,
                          const ncnn::VkMat& snapshot_recurrent)
    {
        VulkanRuntimeState& runtime_state = vulkan_context->runtime_state();
        std::unique_lock<std::mutex> lock(vulkan_context->command_mutex());
        ncnn::VkCompute cmd(vulkan_context->device(),
    ...
    {
    public:
        explicit RecordingGuard(std::span<Implementation* const> _states)
            : states(_states)
        {
        }
...
static bool create_state_storage(ncnn::VkMat& destination,
                                 uint32_t elements,
...
GatedDeltaState_vulkan::create(const std::shared_ptr<VulkanContext>& context,
                               ...
                               const ncnn::Option& opt)
{
    ...
    std::shared_ptr<GatedDeltaState_vulkan> result(new GatedDeltaState_vulkan);
    ...
    if (!create_state_storage(implementation.convolution,
                              static_cast<uint32_t>(convolution_elements),
                              context->blob_allocator())
        || !create_state_storage(implementation.recurrent,
                                 static_cast<uint32_t>(recurrent_elements),
                                 context->blob_allocator())
        || !create_state_storage(implementation.zero_convolution,
                                 static_cast<uint32_t>(convolution_elements),
                                 context->staging_allocator())
        || !create_state_storage(implementation.zero_recurrent,
                                 static_cast<uint32_t>(recurrent_elements),
                                 context->staging_allocator())
        || !fill_zero_staging(implementation.zero_convolution,
                              static_cast<uint32_t>(convolution_elements))
        || !fill_zero_staging(implementation.zero_recurrent,
                              static_cast<uint32_t>(recurrent_elements)))
    {
        return {};
    }
...
bool GatedDeltaState_vulkan::begin_transaction(size_t expected_rows) noexcept
{
#if NCNN_MOE_WITH_VULKAN
    ...
    try
    {
        if (!create_state_storage(implementation.initial_convolution,
                                  static_cast<uint32_t>(convolution_elements),
                                  implementation.vulkan_context->blob_allocator())
            || !create_state_storage(implementation.initial_recurrent,
                                     static_cast<uint32_t>(recurrent_elements),
                                     implementation.vulkan_context->blob_allocator()))
        {
        ...
        {
            if (!create_state_storage(implementation.convolution_snapshots[index],
                                      static_cast<uint32_t>(convolution_elements),
                                      implementation.vulkan_context->blob_allocator())
                || !create_state_storage(implementation.recurrent_snapshots[index],
                                         static_cast<uint32_t>(recurrent_elements),
                                         implementation.vulkan_context->blob_allocator()))
            {
                complete_transaction();
...
bool GatedDeltaState_vulkan::prepare_cpu_state(const std::vector<float>& convolution,
                                               const std::vector<float>& recurrent)
{
#if NCNN_MOE_WITH_VULKAN
    Implementation& implementation = *d;
    const size_t expected_convolution = static_cast<size_t>(implementation.convolution_size)
                                        * implementation.kernel_size;
    const size_t expected_recurrent = static_cast<size_t>(implementation.head_count)
                                      * implementation.head_dimension
                                      * implementation.value_head_dimension;
    ...
    implementation.cpu_convolution.create(static_cast<int>(expected_convolution),
                                          1,
                                          sizeof(float),
                                          implementation.vulkan_context->staging_allocator());
    implementation.cpu_recurrent.create(static_cast<int>(expected_recurrent),
                                        1,
                                        sizeof(float),
                                        implementation.vulkan_context->staging_allocator());
    ...
    std::copy(convolution.begin(),
              convolution.end(),
              static_cast<float*>(implementation.cpu_convolution.mapped_ptr()));
    std::copy(recurrent.begin(),
              recurrent.end(),
              static_cast<float*>(implementation.cpu_recurrent.mapped_ptr()));
    implementation.cpu_convolution.allocator->flush(implementation.cpu_convolution.data);
    implementation.cpu_recurrent.allocator->flush(implementation.cpu_recurrent.data);
...
bool GatedDeltaState_vulkan::prepare_transaction_finish(size_t committed_rows,
        ...
        || implementation.transaction_requires_restore)
    {
        if (committed_rows == 0)
        {
            restored = implementation.transaction_initial_recorded
                       && implementation.restore_snapshot(implementation.initial_convolution,
                                                          implementation.initial_recurrent);
        }
        else if (committed_rows - 1 < implementation.convolution_snapshots.size())
        {
            restored = implementation.restore_snapshot(implementation.convolution_snapshots[committed_rows - 1],
...
bool GatedDeltaState_vulkan::download(std::vector<float>& convolution,
                                      std::vector<float>& recurrent) const
{
    ...
    const size_t convolution_count = static_cast<size_t>(implementation.convolution_size)
                                     * implementation.kernel_size;
    const size_t recurrent_count = static_cast<size_t>(implementation.head_count)
                                   * implementation.head_dimension
                                   * implementation.value_head_dimension;
    ...
    if (convolution_mapped.empty() || recurrent_mapped.empty())
        return false;
    convolution.assign(static_cast<const float*>(convolution_mapped.data),
                       static_cast<const float*>(convolution_mapped.data) + convolution_count);
    recurrent.assign(static_cast<const float*>(recurrent_mapped.data),
...
GatedDeltaNet_vulkan::create(const std::shared_ptr<Bfloat16Linear_vulkan>& fused_input,
                             ...
                             uint64_t optimization_flags)
{
    ...
    std::shared_ptr<GatedDeltaNet_vulkan> result(new GatedDeltaNet_vulkan);
    Implementation& implementation = *result->d;
    ...
    implementation.sigmoid_gate = sigmoid_gate;
    {
        ...
        if (!create_pipeline(implementation.vulkan_context,
                             implementation.opt,
    ...
    implementation.weight_allocator.reset(new ncnn::VkWeightAllocator(implementation.vulkan_context->device()));
    ...
    upload_option.staging_vkallocator = implementation.weight_staging_allocator.get();
    {
        ...
        cmd.record_upload(convolution_model,
...
bool GatedDeltaNet_vulkan::forward_impl(const ActivationBuffer& input,
                                        LayerCache& cache,
                                        ...
                                        bool apply_input_rms_norm) const
{
    ...
    if (!cache.gated_delta_device_state)
    {
        ...
        cache.gated_delta_device_state = GatedDeltaState_vulkan::create(implementation.vulkan_context,
        ...
        if (has_cpu_state
            && !cache.gated_delta_device_state->prepare_cpu_state(cache.gated_delta_convolution,
    ...
    {
        if (!state->begin_transaction(cache.transaction.expected_rows))
            return false;
    }
    ...
    const bool direct_host_input = vulkan_activation_storage_variant(implementation.opt) == 0
                                   && implementation.vulkan_context->support_direct_host_buffer(input.rows() * input.columns() * sizeof(float),
                                                                                                input.dtype());
    ...
    fused_gpu.create(static_cast<int>(implementation.fused_columns),
                     static_cast<int>(input.rows()),
                     sizeof(float),
                     implementation.vulkan_context->blob_allocator());
    ncnn::VkMat recurrent_output_gpu;
    recurrent_output_gpu.create(static_cast<int>(implementation.value_size),
                                static_cast<int>(input.rows()),
                                sizeof(float),
                                implementation.vulkan_context->blob_allocator());
    ...
    {
        implementation.fused_input->record_rms_norm_projection(input_gpu,
                                                               fused_gpu,
                                                               cmd);
    }
    else
    {
        implementation.fused_input->record_scalar_projection(input_gpu,
                                                             fused_gpu,
                                                             cmd);
    }
    ...
    implementation.output_projection->record_scalar_projection(recurrent_output_gpu,
                                                               output_gpu,
                                                               cmd);
...
bool GatedDeltaNet_vulkan::forward(const ActivationBuffer& normalized,
                                   LayerCache& cache,
...
bool GatedDeltaNet_vulkan::forward_input_rms_norm(const ActivationBuffer& input,
                                                  LayerCache& cache,
                                                  ActivationBuffer& projected) const
{
...
GatedDeltaNet_vulkan::forward_batch(std::span<const GatedDeltaBatchEntry_vulkan> entries) const
{
#if NCNN_MOE_WITH_VULKAN
    const Implementation& implementation = *d;
    if (entries.empty())
    ...
    combined_normalized.reset(total_rows,
                              input_columns,
                              false);
    ...
    {
        explicit BatchDeviceStateCreationAttempt(std::vector<LayerCache*>& _created_caches)
    ...
    for (size_t entry_index = 0; entry_index < entries.size(); ++entry_index)
    {
        ...
        std::copy_n(entry.normalized->row(0),
                    input_columns,
    ...
    VulkanTransferSlot& transfer_slot = transfer_lease.slot();
    const bool direct_host_input = vulkan_activation_storage_variant(implementation.opt) == 0
                                   && implementation.vulkan_context->support_direct_host_buffer(total_rows
                                                                                                    * input_columns
                                                                                                    * sizeof(float),
    ...
    combined_projected.reset(total_rows,
                             output_columns,
                             false);
    for (const GatedDeltaBatchEntry_vulkan& entry : entries)
    {
        entry.projected->reset(1,
                               output_columns,
                               false);
    }
    ...
    transaction_recording.mark_submitted();
    if (!copy_staging_to_cpu_batch(transfer_slot.download,
                                   combined_projected))
    {
        mark_nontransaction_existing_states_unknown();
        return GatedDeltaBatchResult_vulkan::Failed;
    ...
    for (size_t entry_index = 0; entry_index < entries.size(); ++entry_index)
    {
        ...
        std::copy_n(combined_projected.row(entry_index),
                    output_columns,
