...
#define NCNN_MOE_CACHE_ENTRY_EXACT_MISS_BIT  1
#define NCNN_MOE_CACHE_ENTRY_DISCARD_BIT     2
...
struct ExpertCache::Entry
{
    ...
    bool has_exact_access = false;
    uint32_t residency_group = ExpertCache::invalid_residency_group;
...
struct ExpertCache::FileRangeReader
{
    struct LoadedRange
    {
      ...
      reserve_cpu_packed_weights(_reserve_cpu_packed_weights)
{
    if (num_io_threads == 0)
    {
        ...
        num_io_threads = std::min(4u, hardware);
    }
    num_io_threads = std::max(1u, num_io_threads);
    try
    {
        workers.reserve(num_io_threads);
        for (uint32_t worker = 0; worker < num_io_threads; ++worker)
            workers.emplace_back(&ExpertCache::worker_loop, this);
    }
    catch (...)
    {
        stop_workers();
...
void ExpertCache::resolve_predictions(uint32_t residency_group, std::span<const std::string_view> demanded_keys)
{
    std::lock_guard<std::mutex> lock(mutex);
...
Result<uint64_t> ExpertCache::stored_size(const TensorData& tensor)
{
...
Result<uint64_t> ExpertCache::packed_weight_size(const TensorData& tensor)
{
...
std::string ExpertCache::make_pair_key(const TensorData& gate_up, const TensorData& down)
{
...
Result<std::shared_ptr<TensorData>> ExpertCache::load_tensor(const TensorData& source, uint64_t& mapped_range_count, uint64_t& mapped_size)
{
...
Result<ExpertVictimPair> ExpertCache::load_independent_pair(const TensorData& gate_up, const TensorData& down, uint64_t& mapped_range_count, uint64_t& mapped_size)
{
...
Result<ExpertVictimPair> ExpertCache::load_interleaved_pair(const TensorData& gate_up, const TensorData& down, uint64_t& mapped_range_count, uint64_t& mapped_size)
{
    const MxFp4FileStorage& gate = *gate_up.mxfp4_file_storage;
    const MxFp4FileStorage& projection = *down.mxfp4_file_storage;
    ...
    if (!one_file)
        return load_independent_pair(gate_up, down, mapped_range_count, mapped_size);
...
Result<ExpertVictimPair> ExpertCache::load_pair(const TensorData& gate_up, const TensorData& down, uint64_t& mapped_range_count, uint64_t& mapped_size)
{
    if (is_file_backed_bfloat16_tensor(gate_up)
        && is_file_backed_bfloat16_tensor(down))
    {
    ...
    bool contiguous = gate.blocks_path == gate.scales_path
                      && gate.blocks_path == projection.blocks_path
                      && gate.blocks_path == projection.scales_path;
    if (contiguous)
    {
...
void ExpertCache::insert_resident_locked(Entry& entry, bool frequent)
{
    if (frequent)
    {
...
void ExpertCache::touch_resident_locked(Entry& entry, bool repeated)
{
    if (entry.arc_list == Entry::ArcList::Recent && repeated)
    {
        arc_frequent.splice(arc_frequent.end(), arc_recent, entry.arc_position);
        arc_recent_size -= entry.resident_size;
        entry.arc_list = Entry::ArcList::Frequent;
        arc_frequent_size += entry.resident_size;
        return;
...
void ExpertCache::erase_ghost_locked(GhostIndex& index, GhostList& list, uint64_t& size, std::string_view key)
{
...
void ExpertCache::add_ghost_locked(const Entry& entry)
{
    ...
    ExpertKeySize record{entry.key, entry.resident_size};
    if (entry.arc_list == Entry::ArcList::Recent)
    {
...
void ExpertCache::trim_ghost_front_locked(GhostIndex& index, GhostList& list, uint64_t& size)
{
    if (list.empty())
        return;
...
void ExpertCache::trim_ghosts_locked()
{
...
void ExpertCache::remove_resident_locked(Entry& entry, bool add_ghost)
{
...
uint64_t ExpertCache::arc_delta(uint64_t required_size, uint64_t numerator, uint64_t denominator) const
{
...
bool ExpertCache::consume_ghost_locked(std::string_view key, uint64_t required_size, bool& frequent, bool& from_frequent_ghost)
{
    frequent = false;
...
bool ExpertCache::evict_one_locked(bool incoming_from_frequent_ghost, bool speculative_admission, uint32_t incoming_group, uint64_t required_size)
{
    ...
    if (victim_cache)
    {
        victim_cache->admit(victim->key, victim->gate_up, victim->down, victim->victim_execution);
    }
    ...
    bool* temporarily_exhausted)
{
    ...
    if (existing != entries.end())
    {
        ...
        if (!speculative
            && has_flag(entry->flags, Entry::Speculative)
            && (entry->state == Entry::State::Queued || entry->state == Entry::State::Loading))
        {
    ...
    for (;;)
    {
        batch.clear();
        {
            ...
            work_available.wait(lock, [this] {
                return stopping || !high_priority.empty() || !low_priority.empty();
            });
            ...
            while (!queue.empty())
            {
                ...
                queue.pop_front();
                if (entry && entry->state == Entry::State::Queued)
                {
                ...
                && !has_flag(flags, ExpertCacheMemoryMapRanges))
            {
                ...
                for (auto iterator = queue.begin(); iterator != queue.end() && batch.size() < maximum_batch_size;)
                {
                    ...
                    if (!candidate
                        || candidate->state != Entry::State::Queued)
                    {
    ...
    ExpertVictimExecutionMetadata victim_execution)
{
    ...
    ready.wait(lock, [&entry] {
        return entry->state == Entry::State::Ready || entry->state == Entry::State::Failed;
    });
    if (entry->state == Entry::State::Failed)
        return entry->error;

    touch_resident_locked(*entry, entry->has_exact_access);
    entry->has_exact_access = true;
    entry->flags &= ~Entry::Speculative;
    ...
    lease.gate_up_operator = &entry->gate_up_operator;
    lease.down_operator = &entry->down_operator;
...
bool ExpertCache::is_ready(const TensorData& gate_up, const TensorData& down, std::string_view prepared_key) const
{
...
ExpertCacheStatistics ExpertCache::statistics() const
{
    ExpertCacheStatistics result;
    std::shared_ptr<ExpertVictimCache> victim;
    {
        std::lock_guard<std::mutex> lock(mutex);
        result.hits = hits;
