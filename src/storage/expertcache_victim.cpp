...
ReuseFilteredExpertVictimCache::ReuseFilteredExpertVictimCache(std::shared_ptr<ExpertVictimCache> _inner, uint32_t _reuse_probe_interval)
...
void ReuseFilteredExpertVictimCache::wait_for_background_work()
{
    inner->wait_for_background_work();
}

ExpertVictimCacheStatistics ReuseFilteredExpertVictimCache::statistics() const
{
...
uint64_t ReuseFilteredExpertVictimCache::capacity() const noexcept
{
    return inner->capacity();
}

bool ReuseFilteredExpertVictimCache::add_size(uint64_t& size, size_t count)
{
...
uint64_t ReuseFilteredExpertVictimCache::payload_size(const std::shared_ptr<const TensorData>& gate_up, const std::shared_ptr<const TensorData>& down)
{
    if (!gate_up || !down)
        return 0;
    uint64_t size = 0;
...
void ReuseFilteredExpertVictimCache::trim_history_locked()
{
...
ShardedExpertVictimCache::ShardedExpertVictimCache(std::vector<std::shared_ptr<ExpertVictimCache>> _shards)
...
void ShardedExpertVictimCache::wait_for_background_work()
{
    for (const auto& item : shards)
        item->wait_for_background_work();
}

ExpertVictimCacheStatistics ShardedExpertVictimCache::statistics() const
{
...
uint64_t ShardedExpertVictimCache::capacity() const noexcept
{
    return total_size;
}

ExpertVictimCache& ShardedExpertVictimCache::shard(const std::string& key) const
{
    const size_t index = std::hash<std::string_view>{}(key) % shards.size();
    return *shards[index];
}

std::shared_ptr<ExpertVictimCache> create_reuse_victim_cache(std::shared_ptr<ExpertVictimCache> inner, uint32_t reuse_probe_interval)
{
    if (!inner)
        return {};
...
std::shared_ptr<ExpertVictimCache> create_sharded_victim_cache(std::vector<std::shared_ptr<ExpertVictimCache>> shards)
{
