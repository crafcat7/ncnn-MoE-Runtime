...
The optional `--enable-vulkan-gated-delta` path follows llama.cpp's Vulkan
Gated DeltaNet structure: fused BF16 input projection, device-resident FP32
convolution/recurrent state, one workgroup-owned sequential state transition,
and Vulkan output projection. It reduces attention wall time substantially on
