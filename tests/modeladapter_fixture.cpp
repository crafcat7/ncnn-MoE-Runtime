...
static Result<void> add_tensor(WeightMapping& mapping, SequentialWeightReader& reader, const std::string& name, std::vector<uint32_t> shape, DType dtype)
{
    ...
    if (!tensor)
        return tensor.error();
    mapping.emplace(name, std::move(tensor).value());
    return {};
}
...
Result<MoeModelDescriptor> FixtureModelAdapter::parse_model(const ModelPackage& package) const
{
...
Result<WeightMapping> FixtureModelAdapter::map_weights(const ModelPackage& package, const MoeModelDescriptor& descriptor) const
{
